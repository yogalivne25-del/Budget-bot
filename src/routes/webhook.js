...
router.post('/', async (req, res) => {
  ...
  if (!user.name && userState[from] !== 'awaiting_name') {
    ...
    reply = `היי ${user.name || ''}! הנה כל מה שאני יודע לעשות 😊\n\n💸 *רישום הוצאה:*\n"קניתי קפה ב-15 שקל"\n"שילמתי דלק 200 שקל"\n"הוצאתי על אוכל 80 שקל"\n\n💰 *רישום הכנסה:*\n"קיבלתי משכורת 8000 שקל"\n"נכנס לי בונוס 500 שקל"\n\n📊 *דוחות:*\n"דוח" - סיכום חודשי מלא\n\n🎯 *תקציב:*\n"תקציב" - לראות את כל התקציבים\n"הגדר תקציב אוכל 1000" - להגדיר תקציב\n\n🗑️ *תיקון טעות:*\n"מחק הוצאה אחרונה"\n\n🌐 *דשבורד:*\n"לינק" - קישור לדשבורד האישי שלך\n\n📅 *אוטומטי:*\nכל יום ראשון אשלח לך סיכום שבועי\nתקבל התראה כשתגיע ל-80% מהתקציב 🟡\nתקבל התראה כשתחרוג מהתקציב 🔴`;
